          ...
          {[0, 1].map(i => (
            <div key={i} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 animate-pulse">
              ...
              {Array.from({ length: 5 }).map((_, j) => (
                <div key={j} className="flex gap-3 py-2.5 border-b border-gray-50 dark:border-gray-800 last:border-0">
                  <div className="flex-1 h-3 bg-gray-100 dark:bg-gray-800 rounded" />
                  <div className="w-10 h-3 bg-gray-100 dark:bg-gray-800 rounded" />
                </div>
              ))}
            </div>
          ))}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-6">
        ...
        <StatCard
          ...
          valueColor={stats.lowStockCount > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-50'}
          icon={
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
      ...
      {items.length === 0 ? (
        ...
        <div className="divide-y divide-gray-50 dark:divide-gray-800">
          {/* Header */}
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-2 text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide">
            <span>Product</span>
            <span>Branch</span>
          ...
          {items.map(item => (
            <div key={item.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
              <div>
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400 self-center">{item.branch.name}</span>
              <span className={`text-sm font-semibold self-center text-right
                ${type === 'in' ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
                {type === 'in' ? '+' : '-'}{item.quantity}
              </span>
              <span className="text-xs text-gray-400 dark:text-gray-500 self-center text-right">
