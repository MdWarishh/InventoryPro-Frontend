...
export default function InvoiceSettingsPage() {
  ...
  const val = newPaymentMode.trim()
  if (!val) return
  const current = (settings as any).customPaymentModes ?? []
  if (current.includes(val)) return
  set('customPaymentModes' as any, [...current, val])
  setNewPaymentMode('')
}

const removePaymentMode = (idx: number) => {
  const current = (settings as any).customPaymentModes ?? []
  set('customPaymentModes' as any, current.filter((_: any, i: number) => i !== idx))
}
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: UpdateSettingsPayload = {
        ...
        placeOfSupplyCode: settings.placeOfSupplyCode,
        dueDateTerms: settings.dueDateTerms,
       ...
       customPaymentModes: (settings as any).customPaymentModes,
      };
  ...
  const handleUpload = async (
    ...
    try {
      ...
      fd.append(
        field === "logo" ? "logo" : field === "qrCode" ? "qrCode" : "signature",
        file,
      );
      ...
      if (field === "logo") updated = await settingsService.uploadLogo(fd);
        ...
        updated = await settingsService.uploadQRCode(fd);
      else updated = await settingsService.uploadAuthorizedSignature(fd);
      setSettings(updated);
      showToast(
        `${field === "logo" ? "Logo" : field === "qrCode" ? "QR Code" : "Signature"} uploaded!`,
        "success",
      );
    } catch {
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 sm:px-6">
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Configure company details, invoice layout, bank info and
              appearance
            </p>
          </div>
          <button
          ...
          {activeTab === "company" && (
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field
                  label="Company Name"
                  hint="Appears at top of every invoice"
                >
                  <Input
                    value={settings.companyName ?? ""}
                    onChange={(e) => set("companyName", e.target.value)}
                    placeholder="e.g. Limra Speech & Hearing Clinic"
                  />
                </Field>
                <Field
                  label="Legal Name"
                  hint="Full registered legal name (optional)"
                >
                  <Input
                ...
                <Field label="GSTIN">
                  <Input
                    ...
                    placeholder="e.g. 07ABAPIO892BIZ5"
                    maxLength={15}
                  />
                </Field>
                <Field label="Phone">
            ...
            <div className="p-6 space-y-8">
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-4">
                  ...
                  <Field label="Reset Counter Monthly">
                    <Select
                      value={settings.resetInvoiceMonthly ? "true" : "false"}
                      onChange={(e) =>
                        set("resetInvoiceMonthly", e.target.value === "true")
                      }
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-4">
                  <Field label="Default GST Rate (%)">
                    <Select
                      ...
                      onChange={(e) =>
                        set("defaultGSTRate", Number(e.target.value))
                      }
                    >
                      {[0, 5, 12, 18, 28].map((r) => (
                  ...
                  <Field label="Place of Supply Code">
                    <Input
                      value={settings.placeOfSupplyCode ?? ""}
                  ...
                  <Field label="Due Date Terms">
                    <Input
                      value={settings.dueDateTerms ?? ""}
                      onChange={(e) => set("dueDateTerms", e.target.value)}
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 mb-4">
                  Invoice create karte waqt dropdown mein ye methods dikhenge.
                </p>
                ...
                <div className="flex gap-2 mb-3">
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") addPaymentMode();
                    }}
                    placeholder="e.g. Cheque, Online Transfer, NEFT..."
                    className="flex-1"
                  />
                  <button
                    type="button"
                    ...
                    className="px-4 py-2 text-sm font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-700
        text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                ...
                <div className="flex flex-wrap gap-2">
                  {(settings.customPaymentModes ?? []).length === 0 ? (
                    <p className="text-sm text-slate-400 dark:text-slate-500 italic">
                      Koi payment method nahi add kiya abhi.
                    </p>
                  ) : (
                    (settings.customPaymentModes ?? []).map((mode, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-slate-200
            dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm font-medium
            text-slate-700 dark:text-slate-200"
                      >
                        {mode}
                        <button
                          type="button"
                          onClick={() => removePaymentMode(idx)}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-5 mt-4">
                  <Field
                    ...
                    hint="Printed at bottom of each invoice. Enter dabao next line ke liye."
                  >
                    <Textarea
          ...
          {activeTab === "bank" && (
            <div className="p-6 space-y-6">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 -mt-4">
                These details appear on every invoice so customers can transfer
                payments directly.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <Field label="Branch Name">
                  <Input
                    ...
                    placeholder="New Friends Colony, N.D."
                  />
                </Field>
              </div>
              {settings.bankAccountNumber && (
                <div className="mt-2 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-xl p-4 text-sm">
                  ...
                  {settings.bankIFSC && (
                    <div className="text-slate-600 dark:text-slate-300">
                      IFSC: <strong>{settings.bankIFSC}</strong>
                    </div>
                  )}
                  {settings.bankBranch && (
                    <div className="text-slate-600 dark:text-slate-300">
          ...
          {activeTab === "media" && (
            <div className="p-6 space-y-5">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 -mt-4">
                Upload images that appear on your invoices. PNG with transparent
                background recommended.
              </p>
              <ImageUploadCard
                label="Company Logo"
                hint="Shown top-left on every invoice. Recommended: square PNG, min 200×200px."
              ...
              />
              <ImageUploadCard
                label="QR Code for Payments"
                hint="Shown at bottom of invoice for online payment scanning. Square PNG."
                current={settings.qrCodeImage}
                fieldName="qrCode"
                onUpload={handleUpload}
                uploading={uploading}
              />
              <ImageUploadCard
                ...
                hint="Shown bottom-right of invoice. Transparent background PNG preferred."
                current={settings.authorizedSignature}
                fieldName="signature"
                onUpload={handleUpload}
                uploading={uploading}
              />
          ...
          {activeTab === "appearance" && (
            <div className="p-6 space-y-8">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 mb-4">
                  Colors used in the invoice footer bar and accents.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  <Field label="Primary Color">
                    <div className="flex gap-2">
                      <div
                        className="w-10 h-10 rounded-lg border border-slate-200 dark:border-slate-700 flex-shrink-0 cursor-pointer overflow-hidden"
                        style={{
                  ...
                  <Field
                    ...
                    hint="Bottom color bar on invoice"
                  >
                    <div className="flex gap-2">
                      <div
                        className="w-10 h-10 rounded-lg border border-slate-200 dark:border-slate-700 flex-shrink-0 cursor-pointer overflow-hidden"
                ...
                <div className="mt-4 rounded-xl overflow-hidden">
                  <div
                    className="px-4 py-3 text-white text-xs font-bold grid grid-cols-3 items-center gap-3"
                    style={{
                      background:
                        settings.footerColor ??
                        settings.primaryColor ??
                  ...
                  >
                    ...
                    <div className="flex justify-center">
                      {settings.logo ? (
                        <img
                          src={settings.logo}
                          className="w-10 h-10 rounded-full object-contain border-2 border-white/50"
                        />
                      ) : (
