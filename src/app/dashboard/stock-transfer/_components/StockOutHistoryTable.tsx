...
interface Props {
  ...
  fetching: boolean
  onEdit: (record: StockOutRecord) => void
  onDeleteSuccess?: () => void
}

export default function StockOutHistoryTable({ items, fetching, onEdit, onDeleteSuccess }: Props) {
  const queryClient = useQueryClient()
  const [deleteTarget, setDeleteTarget] = useState<StockOutRecord | null>(null)

  const deleteMutation = useMutation({
    mutationFn: (id: string) => stockService.deleteStockOut(id),
    onSuccess: () => {
      toast.success('Stock-out record deleted successfully.')
      queryClient.invalidateQueries({ queryKey: ['stock-history', 'out'] })
          ...
          {fetching ? (
            ...
            <TableRow key={r.id} className="border-border group">
              ...
              <TableCell className="text-sm text-muted-foreground">
                {r.customerName || <span className="text-border">—</span>}
              </TableCell>
              <TableCell>
