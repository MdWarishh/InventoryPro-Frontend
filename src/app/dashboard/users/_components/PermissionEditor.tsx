...
const GROUP_COLORS: Record<string, { dot: string; label: string; header: string }> = {
  Core:       { dot: 'bg-violet-500',  label: 'text-violet-700',  header: 'bg-violet-50 border-violet-100' },
  Catalog:    { dot: 'bg-sky-500',     label: 'text-sky-700',     header: 'bg-sky-50 border-sky-100' },
  Operations: { dot: 'bg-emerald-500', label: 'text-emerald-700', header: 'bg-emerald-50 border-emerald-100' },
  Other:      { dot: 'bg-amber-500',   label: 'text-amber-700',   header: 'bg-amber-50 border-amber-100' },
}
...
export function PermissionEditor({ permissions, onChange }: PermissionEditorProps) {
  ...
  const update = (module: ModuleKey, key: keyof Omit<Permission, 'module'>, value: boolean) => {
    ...
    // Logic: uncheck view → uncheck all others
    if (key === 'canView' && !value) {
      updated = { ...updated, canCreate: false, canEdit: false, canDelete: false }
    }
    // Logic: check create/edit/delete → auto-check view
  ...
  return (
    <div className="space-y-3">
      ...
      {GROUPS.map(group => {
        ...
        return (
          <div key={group} className="rounded-xl border border-slate-200/80 overflow-hidden">
            {/* Group header with select-all per column */}
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    idx !== groupModules.length - 1 && 'border-b border-slate-100',
                    isActive ? 'bg-white' : 'bg-slate-50/60',
                    'hover:bg-slate-50'
                  )}
                  style={{ gridTemplateColumns: '1fr repeat(4, 44px)' }}
                >
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    {isActive && (
                      <span className="shrink-0 w-1 h-1 rounded-full bg-emerald-400" />
                    )}
                  </div>
